...
import { Truck, Package, ChevronRight, Activity, TrendingUp, Users } from 'lucide-react';
...
export const DashboardPage: React.FC = () => {
    ...
    const { data: stats } = useQuery({
        ...
        queryFn: async () => {
            ...
            const res = await axios.get(`${config.api.baseUrl}/platform-admin/users/stats`, {
    ...
    return (
        <div className="space-y-8">
            <div className="flex flex-col gap-1">
                <h1 className="text-4xl font-bold tracking-tight">{isAdmin ? 'Platform Oversight' : 'Overview'}</h1>
                <p className="text-slate-400 font-medium">Welcome back, {user?.displayName || user?.email}</p>
            </div>

            {isAdmin ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <Card className="p-6 space-y-4 shadow-xl shadow-primary/5 border-primary/10" hoverable>
                        <div className="flex justify-between items-start">
                            <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Platform Revenue</p>
                            <TrendingUp size={18} className="text-primary" />
                        </div>
                        <div className="space-y-1">
                            <p className="text-3xl font-bold text-primary">{(stats?.financials?.platformRevenue || 0).toLocaleString()}</p>
                            <p className="text-[10px] text-slate-500 font-bold">NOK TOTAL COMMISSION</p>
                        </div>
                    </Card>

                    <Card className="p-6 space-y-4" hoverable>
                        <div className="flex justify-between items-start">
                            <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Marketplace GMV</p>
                            <Activity size={18} className="text-blue-400" />
                        </div>
                        <div className="space-y-1">
                            <p className="text-3xl font-bold">{(stats?.financials?.totalGMV || 0).toLocaleString()}</p>
                            <p className="text-[10px] text-slate-500 font-bold">GROSS MERCHANDISE VOLUME</p>
                        </div>
                    </Card>

                    <Card className="p-6 space-y-4" hoverable>
                        <div className="flex justify-between items-start">
                            <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Companies</p>
                            <Package size={18} className="text-orange-400" />
                        </div>
                        <div className="space-y-1">
                            <p className="text-3xl font-bold">{stats?.financials?.totalCompanies || 0}</p>
                            <p className="text-[10px] text-slate-500 font-bold">REGISTERED ENTITIES</p>
                        </div>
                    </Card>
                    ...
                    <Card className="p-6 space-y-4" hoverable>
                        ...
                        <div className="space-y-1">
                            ...
                            <p className="text-[10px] text-slate-500 font-bold">{stats?.activeToday || 0} ACTIVE TODAY</p>
                        </div>
                    </Card>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <Card className="p-6 space-y-4" hoverable>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <p className="text-xl font-bold">Company Admin</p>
                        </div>
                    </Card>
