...
export class PlatformAdminUserService {
  ...
  async getUserStatistics(): Promise<UserStatistics> {
    ...
    try {
      ...
      const byStatus: Record<string, number> = {
        ...
        'SUSPENDED': 0,
        'BANNED': 0,
        'PENDING_VERIFICATION': unverifiedUsers,
        'DEACTIVATED': 0
      };
      ...
      const stats: UserStatistics = {
        ...
        bannedUsers: byStatus['BANNED'],
        pendingVerification: unverifiedUsers,
        verifiedUsers,
        flaggedUsers: 0, // Would need to implement flagging system
        newUsersToday,
        newUsersThisWeek,
        newUsersThisMonth,
  ...
  private transformUserToPlatformUser(user: any): PlatformUser {
    ...
    const status = user.status || (user.emailVerified ? 'ACTIVE' : 'PENDING_VERIFICATION');
  ...
  async updateUserStatus(userId: string, status: PlatformUser['status'], reason: string, updatedBy: string): Promise<PlatformUser> {
    try {
      ...
      // If status is being set to ACTIVE, and it wasn't verified, mark as verified
      if (status === 'ACTIVE' && !currentUser.emailVerified) {
        updateData.emailVerified = true;
      }

      // Handle suspension metadata
      if (status === 'SUSPENDED' || status === 'BANNED') {
        updateData.suspendedAt = new Date();
        updateData.suspendedBy = updatedBy;
        updateData.suspensionReason = reason;
      } else if (status === 'ACTIVE' || status === 'DEACTIVATED') {
      ...
      await prisma.auditLog.create({
        data: {
          ...
          entityId: userId,
          changes: {
            before: { status: currentUser.status, emailVerified: currentUser.emailVerified },
            after: { status, emailVerified: updateData.emailVerified || currentUser.emailVerified }
          },
      ...
      return this.transformUserToPlatformUser(updatedUser);
    } catch (error) {
