...
const UserManagementPanel: React.FC<UserManagementPanelProps> = ({
  ...
  const handleStatusUpdate = async (userId: string, newStatus: string, reason: string = 'Administrative update') => {
    try {
      setLoading(true);
      setError(null);

      const response = await fetch(getApiUrl(`/platform-admin/users/${userId}/status`), {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input defaultValue={selectedUserForEdit.firstName} disabled />
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-1">Last Name</label>
                    <Input defaultValue={selectedUserForEdit.lastName} disabled />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">Email</label>
                  <Input defaultValue={selectedUserForEdit.email} disabled />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">User Status</label>
                  <select
                    id="user-status-select"
                    defaultValue={selectedUserForEdit.status}
                ...
                <div>
                  <label className="block text-sm font-medium mb-1">Update Reason</label>
                  <Input id="status-update-reason" placeholder="Enter reason for status change..." />
                </div>
                <div className="flex justify-end space-x-2">
                  <Button variant="outline" onClick={() => setSelectedUserForEdit(null)}>Cancel</Button>
                  <Button onClick={async () => {
                    const statusSelect = document.getElementById('user-status-select') as HTMLSelectElement;
                    const reasonInput = document.getElementById('status-update-reason') as HTMLInputElement;
                    const newStatus = statusSelect.value;
                    const reason = reasonInput.value || 'Administrative update';

                    if (newStatus !== selectedUserForEdit.status) {
                      try {
                        await handleStatusUpdate(selectedUserForEdit.id, newStatus, reason);
