...
import { ListingPerformanceCard } from '../components/listings/ListingPerformanceCard';
...
export default function VehicleListingsPage() {
  ...
  // Fetch performance metrics for all listings
  const { data: performanceMetrics } = useQuery<{ vehicles: ListingPerformanceMetrics[] }>({
    queryKey: ['companyListingsPerformance'],
    queryFn: async () => {
      const validToken = await tokenManager.getValidToken();
      return apiClient.get<{ vehicles: ListingPerformanceMetrics[] }>('/listings/analytics/company', validToken);
    },
    enabled: !!token && !!listings && listings.length > 0,
  });
  ...
  return (
    <div className="min-h-screen ds-bg-page">
      ...
      <UserStateGuard 
        ...
        loadingMessage="Loading your vehicle listings..."
      >
        ...
        {!isLoading && !error && listings && listings.length > 0 && (
          <div className="space-y-6">
            ...
            {performanceMetrics && performanceMetrics.vehicles.length > 0 && (
              <div>
                <h2 className="text-xl font-semibold ds-text-gray-900 mb-4">Performance Overview</h2>
                <Grid cols={{ base: 1, sm: 2, lg: 3 }} gap="lg">
                  {performanceMetrics.vehicles.map((metrics) => {
                    const listing = listings.find(l => l.id === metrics.listingId);
                    return listing ? (
                      <ListingPerformanceCard
                        key={metrics.listingId}
                        metrics={metrics}
            ...
            {/* Listings Grid */}
            <div>
              <h2 className="text-xl font-semibold ds-text-gray-900 mb-4">All Listings</h2>
              <Grid cols={{ base: 1, sm: 2, lg: 3 }} gap="lg">
